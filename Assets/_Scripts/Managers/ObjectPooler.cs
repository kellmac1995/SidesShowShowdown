...
public class ObjectPooler : MonoBehaviour
{
    ...
    {
        // TODO make each pool have a scene they spawn in.
    ...
    /// <summary>
    /// Returns the gameobject(prefab) with from a pool with the given name.
    /// </summary>
    /// <param name="_name">Name of the pool to find.</param>
    /// <returns></returns>
    public GameObject GetPooledPrefab(string _name)
    {

        for (int i = 0; i < Pools.Count; i++)
        {
            var pool = Pools[i];
            if (pool.PoolName == _name)
                return pool.ObjectToPool;
        }

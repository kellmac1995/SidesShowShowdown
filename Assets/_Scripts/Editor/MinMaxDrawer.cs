...
public class MinMaxDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.propertyType == SerializedPropertyType.Vector2)
        {
            ...
            //EditorGUI.LabelField(position, minLimit.ToString());
            ...
            if (minMax.ShowDebugValues || minMax.ShowEditRange)
            {
                ...
                Vector2Int val = new Vector2Int(Mathf.RoundToInt(minValue), Mathf.RoundToInt(maxValue));
                //val = EditorGUI.Vector4Field(position, "MinLimit/MinVal/MaxVal/MaxLimit", val);
                val = EditorGUI.Vector2IntField(position, "Min/Max Value", val);
